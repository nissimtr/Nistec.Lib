...
namespace Nistec.Drawing
{
    ...
    static public bool UsingCustomColor
      ...
      { 
		...
		static public Color VSNetPressedColor
		{
			get
			{
				if ( useCustomColor && pressedColor != Color.Empty )
					...
					return CalculateColor(SystemColors.Highlight, ColorUtils.VSNetSelectionColor, 70);
			}
			set
			{
				// Flag that we are going to use custom colors instead
        ...
        private static Color BlackDark = Color.FromArgb(0x30, 0x30, 0x30);
        private static Color BlackDarkDark = Color.FromArgb(0x20, 0x20, 0x20);
        private static Color BlackLight = Color.FromArgb(0x80, 0x80, 0x80);
        private static Color BlackLightLight = Color.FromArgb(0xc0, 0xc0, 0xc0);
        private static Color WhiteDark = Color.FromArgb(160, 160, 160);
        private static Color WhiteDarkDark = Color.FromArgb(0x60, 0x60, 0x60);
        ...
        private static Color WhiteLightLight = Color.FromArgb(250, 250, 250);
         ...
         public static int GreyScale(Color color)
        {
            return ((((0xde * color.R) + (0x2c3 * color.G)) + (0x47 * color.B)) / 0x3e8);
        }
		#endregion
		...
		static public Color ColorFromRGBString( string text )
		{
			...
			try
			{
				...
				else
				{
					...
					bool isKnown = ColorUtils.IsKnownColor( rgbColor, ref knownColor, true);
					if ( !isKnown )
						isKnown = ColorUtils.IsSystemColor(rgbColor, ref knownColor);
					if ( isKnown )
						rgbColor = knownColor;
				}
			}
